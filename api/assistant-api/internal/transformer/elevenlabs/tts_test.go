...
func TestElevenLabsTTSReadLoopConnectionOwnership(t *testing.T) {
	for _, state := range []string{"current", "interrupted", "replaced"} {
		t.Run(state, func(t *testing.T) {
			...
			remote := waitElevenLabsServerConnection(t, serverConnections)
			t.Cleanup(func() { _ = remote.Close() })

			ctx, cancel := context.WithCancel(context.Background())
			collector := testutil.NewPacketCollector()
			transformer := &elevenlabsTTS{
				ctx:           ctx,
				ctxCancel:     cancel,
			...
			require.NoError(t, remote.WriteJSON(map[string]interface{}{
				"audio": base64.StdEncoding.EncodeToString([]byte{1, 2, 3, 4}), "isFinal": true,
			}))
			...
			case "replaced":
				...
				replacementRemote := waitElevenLabsServerConnection(t, serverConnections)
