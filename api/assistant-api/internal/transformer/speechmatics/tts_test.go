...
type speechmaticsTestTransport func(*http.Request) (*http.Response, error)

func (transport speechmaticsTestTransport) RoundTrip(request *http.Request) (*http.Response, error) {
	return transport(request)
}

type speechmaticsTestBody struct {
	read       func([]byte) (int, error)
	closed     chan struct{}
	closeCount atomic.Int32
}

func (body *speechmaticsTestBody) Read(buffer []byte) (int, error) { return body.read(buffer) }

func (body *speechmaticsTestBody) Close() error {
...
func newSpeechmaticsTTSTest(t *testing.T, transport speechmaticsTestTransport) (*speechmaticsTTS, *testutil.PacketCollector) {
	t.Helper()
	client := http.DefaultClient
	http.DefaultClient = &http.Client{Transport: transport}
	t.Cleanup(func() { http.DefaultClient = client })
	collector := testutil.NewPacketCollector()
	transformer, err := NewSpeechmaticsTextToSpeech(context.Background(), testutil.NewTestLogger(),
		testutil.BuildCredential(map[string]string{"key": "test-key"}),
		collector.OnPacket, utils.Option{})
	require.NoError(t, err)
	tts := transformer.(*speechmaticsTTS)
	t.Cleanup(func() { require.NoError(t, tts.Close(context.Background())) })
	return tts, collector
}

func TestSpeechmaticsTTSTerminalResult(t *testing.T) {
	for _, scenario := range []string{"eof", "read_error", "transport_error", "status_error", "service_unavailable"} {
		t.Run(scenario, func(t *testing.T) {
			synctest.Test(t, func(t *testing.T) {
				body := &speechmaticsTestBody{closed: make(chan struct{})}
				...
				tts, collector := newSpeechmaticsTTSTest(t, func(request *http.Request) (*http.Response, error) {
					calls.Add(1)
					assert.Equal(t, http.MethodPost, request.Method)
					assert.Equal(t, SPEECHMATICS_TTS_URL+"/"+SPEECHMATICS_DEFAULT_VOICE+"?output_format=pcm_16000", request.URL.String())
...
func TestSpeechmaticsTTSConfiguredVoice(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		tts, collector := newSpeechmaticsTTSTest(t, func(request *http.Request) (*http.Response, error) {
			assert.Equal(t, SPEECHMATICS_TTS_URL+"/configured-voice?output_format=pcm_16000", request.URL.String())
			return &http.Response{StatusCode: http.StatusOK, Body: io.NopCloser(strings.NewReader("audio")), Header: make(http.Header)}, nil
		})
		tts.mdlOpts = utils.Option{"speak.voice.id": "configured-voice"}
		require.NoError(t, tts.Transform(context.Background(), internal_type.TextToSpeechTextPacket{ContextID: "active", Text: "hello"}))
		require.NoError(t, tts.Transform(context.Background(), internal_type.TextToSpeechDonePacket{ContextID: "active"}))
		synctest.Wait()
		require.Len(t, collector.EndPackets(), 1)
...
func TestSpeechmaticsTTSCancellation(t *testing.T) {
...
func TestSpeechmaticsTTSCloseJoinsAudioCallback(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		tts, collector := newSpeechmaticsTTSTest(t, func(*http.Request) (*http.Response, error) {
			return &http.Response{StatusCode: http.StatusOK, Body: io.NopCloser(strings.NewReader("audio")), Header: make(http.Header)}, nil
		})
...
func TestSpeechmaticsTTSTerminalClaimPrecedesLaterInterruption(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		tts, collector := newSpeechmaticsTTSTest(t, func(*http.Request) (*http.Response, error) {
			return &http.Response{StatusCode: http.StatusOK, Body: io.NopCloser(strings.NewReader("audio")), Header: make(http.Header)}, nil
		})
...
func TestSpeechmaticsTTSInterruptDiscardsBufferedText(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		var calls atomic.Int32
		tts, collector := newSpeechmaticsTTSTest(t, func(*http.Request) (*http.Response, error) {
			calls.Add(1)
