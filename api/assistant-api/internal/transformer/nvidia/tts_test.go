...
type nvidiaTestTransport func(*http.Request) (*http.Response, error)

func (transport nvidiaTestTransport) RoundTrip(request *http.Request) (*http.Response, error) {
	return transport(request)
}

type nvidiaTestBody struct {
	read       func([]byte) (int, error)
	closed     chan struct{}
	closeCount atomic.Int32
}

func (body *nvidiaTestBody) Read(buffer []byte) (int, error) { return body.read(buffer) }

func (body *nvidiaTestBody) Close() error {
...
func newNvidiaTTSTest(t *testing.T, transport nvidiaTestTransport) (*nvidiaTTS, *testutil.PacketCollector) {
	t.Helper()
	client := http.DefaultClient
	http.DefaultClient = &http.Client{Transport: transport}
	t.Cleanup(func() { http.DefaultClient = client })
	collector := testutil.NewPacketCollector()
	transformer, err := NewNvidiaTextToSpeech(context.Background(), testutil.NewTestLogger(),
		testutil.BuildCredential(map[string]string{"key": "test-key", "function_id": "test-function"}),
		collector.OnPacket, utils.Option{})
	require.NoError(t, err)
	tts := transformer.(*nvidiaTTS)
	t.Cleanup(func() { require.NoError(t, tts.Close(context.Background())) })
	return tts, collector
}

func TestNvidiaTTSTerminalResult(t *testing.T) {
	for _, scenario := range []string{"eof", "read_error", "transport_error", "status_error"} {
		t.Run(scenario, func(t *testing.T) {
			synctest.Test(t, func(t *testing.T) {
				body := &nvidiaTestBody{closed: make(chan struct{})}
				...
				tts, collector := newNvidiaTTSTest(t, func(request *http.Request) (*http.Response, error) {
					calls.Add(1)
					assert.Equal(t, http.MethodPost, request.Method)
					assert.Equal(t, "https://api.nvcf.nvidia.com/v2/nvcf/pexec/functions/test-function", request.URL.String())
					assert.Equal(t, "Bearer test-key", request.Header.Get("Authorization"))
					assert.Equal(t, "test-function", request.Header.Get("NVCF-INPUT-ASSET-REFERENCES"))
					assert.Equal(t, "application/json", request.Header.Get("Content-Type"))
					var payload map[string]interface{}
					assert.NoError(t, json.NewDecoder(request.Body).Decode(&payload))
					assert.Equal(t, map[string]interface{}{
						"text": "hello world", "voice_name": NVIDIA_DEFAULT_VOICE,
						"language_code": NVIDIA_DEFAULT_LANGUAGE, "encoding": "LINEAR_PCM", "sample_rate": float64(16000),
					}, payload)
...
func TestNvidiaTTSCancellation(t *testing.T) {
...
func TestNvidiaTTSCloseJoinsAudioCallback(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		tts, collector := newNvidiaTTSTest(t, func(*http.Request) (*http.Response, error) {
...
func TestNvidiaTTSTerminalClaimPrecedesLaterInterruption(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		tts, collector := newNvidiaTTSTest(t, func(*http.Request) (*http.Response, error) {
			return &http.Response{StatusCode: http.StatusOK, Body: io.NopCloser(strings.NewReader("audio")), Header: make(http.Header)}, nil
		})
...
func TestNvidiaTTSInterruptDiscardsBufferedText(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		var calls atomic.Int32
		tts, collector := newNvidiaTTSTest(t, func(*http.Request) (*http.Response, error) {
			calls.Add(1)
