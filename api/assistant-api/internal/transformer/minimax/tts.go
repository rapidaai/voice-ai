	...
	ct.onPacket(
		...
		internal_type.ObservabilityLogRecordPacket{
			...
			Record: observability.RecordLog{
				...
				Message: "minimax-tts: initialization completed",
				Attributes: observability.Attributes{
					"component": observability.ComponentTTS.String(),
					"provider":  ct.Name(),
					"model":     observability.AttributeValue(ct.GetModel()),
				},
				OccurredAt: time.Now(),
			},
		})
...
func (t *minimaxTTS) streamHTTPTTS(ctx context.Context, text, contextID string, startedAt time.Time) {
	...
	defer func() {
		...
		if synthesisError != nil {
			t.onPacket(internal_type.TextToSpeechErrorPacket{
				ContextID: contextID, Error: synthesisError, Type: errorType,
			})
			return
		}
		t.onPacket(
	...
	body, err := json.Marshal(map[string]interface{}{
		"stream": true, "model": t.GetModel(), "text": text,
		"voice_setting": map[string]interface{}{"voice_id": t.GetVoice()},
		"audio_setting": map[string]interface{}{"format": "pcm", "sample_rate": 16000},
	})
	if err != nil {
		synthesisError = fmt.Errorf("minimax-tts: encode request: %w", err)
		return
	}
	request, err := http.NewRequestWithContext(ctx, http.MethodPost,
		t.GetAPIURL(), bytes.NewReader(body))
	if err != nil {
		synthesisError = fmt.Errorf("minimax-tts: create request: %w", err)
		return
	}
	...
	if err != nil {
		synthesisError = fmt.Errorf("minimax-tts: send request: %w", err)
		return
	}
	...
	if response.StatusCode != http.StatusOK {
		synthesisError = fmt.Errorf("minimax-tts: unexpected status code: %d", response.StatusCode)
		return
	}
	...
	for scanner.Scan() {
		...
		var event minimax_internal.MiniMaxTextToSpeechSSEResponse
		if err := json.Unmarshal([]byte(data), &event); err != nil {
			errorType = internal_type.TTSUnknownError
			synthesisError = fmt.Errorf("minimax-tts: parse SSE data: %w", err)
			return
		}
		if event.Data.Audio == "" {
			continue
		}
		audio, err := hex.DecodeString(event.Data.Audio)
		if err != nil {
			errorType = internal_type.TTSUnknownError
			synthesisError = fmt.Errorf("minimax-tts: decode hex audio: %w", err)
			return
		}
	...
	if err := scanner.Err(); err != nil {
		synthesisError = fmt.Errorf("minimax-tts: read SSE response: %w", err)
	}
}
