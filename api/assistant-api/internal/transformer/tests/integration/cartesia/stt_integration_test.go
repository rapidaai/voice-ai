...
func TestCartesiaSTTLifecycle(t *testing.T) {
	...
	stt, err := cartesia.NewSpeechToText(cartesia.WithContext(ctx), cartesia.WithLogger(logger), cartesia.WithCredential(
		testutil.BuildCredential(pcfg.Credential)), cartesia.WithOnPacket(
		collector.OnPacket), cartesia.WithOptions(
...
func TestCartesiaSTTReconnect(t *testing.T) {
	...
	for attempt := 0; attempt < 2; attempt++ {
		...
		stt, err := cartesia.NewSpeechToText(cartesia.WithContext(ctx), cartesia.WithLogger(logger), cartesia.WithCredential(cred), cartesia.WithOnPacket(collector.OnPacket), cartesia.WithOptions(opts))
