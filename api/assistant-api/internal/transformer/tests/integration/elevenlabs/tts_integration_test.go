...
import (
	...
	elevenlabs "github.com/rapidaai/api/assistant-api/internal/transformer/elevenlabs"
...
func TestElevenLabsTTSLifecycle(t *testing.T) {
	...
	tts, err := elevenlabs.NewElevenlabsTextToSpeech(ctx, logger, cred, collector.OnPacket, opts)
...
func TestElevenLabsTTSFlow_DeltaInterruptDeltaDone(t *testing.T) {
	...
	tts, err := elevenlabs.NewElevenlabsTextToSpeech(ctx, logger,
	...
	// Phase 3: a new message on the session connection.
	collector.Clear()
	require.NoError(t, tts.Transform(ctx, internal_type.TextToSpeechTextPacket{
