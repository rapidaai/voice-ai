...
func TestSarvamSTTLifecycle(t *testing.T) {
	...
	stt, err := sarvam.NewSpeechToText(sarvam.WithContext(ctx), sarvam.WithLogger(logger), sarvam.WithCredential(cred), sarvam.WithOnPacket(collector.OnPacket), sarvam.WithOptions(opts))
...
func TestSarvamSTTAudioAcceptance(t *testing.T) {
	...
	stt, err := sarvam.NewSpeechToText(sarvam.WithContext(ctx), sarvam.WithLogger(logger), sarvam.WithCredential(
		testutil.BuildCredential(pcfg.Credential)), sarvam.WithOnPacket(
		collector.OnPacket), sarvam.WithOptions(
