...
func TestDeepgramSTTLifecycle(t *testing.T) {
	...
	stt, err := deepgram.NewSpeechToText(
		deepgram.WithContext(ctx),
		deepgram.WithLogger(logger),
		deepgram.WithCredential(cred),
		deepgram.WithOnPacket(collector.OnPacket),
		deepgram.WithOptions(opts),
	)
...
func TestDeepgramSTTCloseWhileStreaming(t *testing.T) {
	...
	stt, err := deepgram.NewSpeechToText(
		deepgram.WithContext(ctx),
		deepgram.WithLogger(logger),
		deepgram.WithCredential(testutil.BuildCredential(pcfg.Credential)),
		deepgram.WithOnPacket(collector.OnPacket),
		deepgram.WithOptions(testutil.BuildOptions(pcfg.Options)),
	)
	require.NoError(t, err)
	assertSTTInitMetric(t, collector)
