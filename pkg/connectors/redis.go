...
func (redisC *redisConnector) Cmd(ctx context.Context, cmd string, args []string) *RedisResponse {
	start := time.Now()
	redisC.logger.Debugf("started executing redis cmd %s", cmd)
	...
	redisC.logger.Debugf("ending executing redis cmd and returning result with result %v", val)
	redisC.logger.Infof("returning redis `Cmd` for command %s result with time %s", cmd, time.Since(start))
...
func (redisC *redisConnector) Cmds(ctx context.Context, cmd string, args *[]string) *RedisResponse {
	...
	redisC.logger.Infof("returning redis `Cmds` result with count %d time %s", len(result), time.Since(start))
